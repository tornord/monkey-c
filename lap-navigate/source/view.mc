...
function haversineFormula(lat1, lon1, lat2, lon2){  // generally used geo measurement function
...
class View extends WatchUi.DataField {
    ...
    function initialize() {
        ...
        appVer = appName.substring(i, appName.length());
    }
    ...
    function onUpdate(dc) {
        ...
        dc.drawText(dc.getWidth() / 2, dc.getHeight() * h, font, s0, Graphics.TEXT_JUSTIFY_CENTER|Graphics.TEXT_JUSTIFY_VCENTER);
        dc.drawText(dc.getWidth() / 2, dc.getHeight() * (h + dh), font, s1, Graphics.TEXT_JUSTIFY_CENTER|Graphics.TEXT_JUSTIFY_VCENTER);
    }
}